...
const TopSellers = () => {
    ...
    useEffect(() => {
        const productcopy = products.filter(item => item.section === "September Buzz");
  ...
  `;
    ...
    return (
        <div className='mt-10 mb-[10px] pt-[5px] pl-2 pr-2 bg-custom-gradient cursor-pointer'>
            <div className='w-full flex justify-center uppercase font-[500]'>
                <h1>september buzz</h1>
            </div>
            <div className='w-full mt-5'>
                <Slider {...settings} className='flex justify-center'>
                    {productData.map(product => (
                        <Link to={`/product/${product._id}`} key={product._id}>
                            <div className=' flex justify-center items-center px-0 m-0 md:ml-0 md:px-6' >
                                <Card className='border border-grey-500' sx={{ maxWidth: 255, }}>
                                    <CardMedia
                                        sx={{
                                            width: 255, transition: 'transform 0.3s ease-in-out', height: '345px',
                                            '&:hover': {
